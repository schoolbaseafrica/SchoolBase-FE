        ...
        <TableBody>
          {students?.map((student: AttendanceStudent, index: number) => (
            <TableRow key={student.student_id}>
              ...
              <TableCell className="text-text-secondary text-center">
                {data.class_name || data.class_id}
              </TableCell>
              <TableCell className="text-center">
                <span
                  className={`inline-block rounded-full px-3 py-1 text-xs font-semibold ${getStatusBadgeClass(student.status || "NOT_MARKED")}`}
                >
                  {student.status ? student.status.replace("_", " ") : "Not Marked"}
                </span>
              </TableCell>
              <TableCell className="text-text-secondary text-center">
                {student.check_in_time
                  ? format(new Date(student.check_in_time), "HH:mm:ss")
                  : "-"}
              </TableCell>
              <TableCell className="text-text-secondary text-center">
                {student.check_out_time
                  ? format(new Date(student.check_out_time), "HH:mm:ss")
                  : "-"}
              </TableCell>
            </TableRow>
          ))}
