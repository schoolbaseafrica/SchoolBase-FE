...
const Attendance = () => {
  ...
  // Fetch all active classes (exclude archived, get all pages)
  ...
  const classes = useMemo(() => {
    ...
    return classesData.items.flatMap((group: ClassItem) =>
      group.classes.map((cls: { id: string; arm: string }) => ({
        id: cls.id,
  ...
  const className = useMemo(() => {
    const foundClass = classes.find(
      (cls: { id: string; name: string }) => cls.id === initialClassId
    )
