...
interface ExistingClassesProps {
...
const ExistingClasses = ({
  ...
  const reactivateClasses = useReactivateClass().mutateAsync
  const [isLoading, setIsLoading] = useState(false)
  const router = useRouter()
  const [toDelete, setToDelete] = useState<string>()
  const [toReactivate, setToReactivate] = useState<string>()
  ...
  return (
    <article className="py-5">
      ...
      <DeleteConfirmationDialog
        ...
        title="Archive Class"
        description="This will archive the class. You can reactivate it later from the archived classes view."
      ...
      <DeleteConfirmationDialog
        open={!!toReactivate}
        onOpenChange={(open) => !open && setToReactivate(undefined)}
        onConfirm={() => handleReactivate(toReactivate as string)}
        title="Reactivate Class"
        description="This will restore the class and make it active again."
  ...
  async function handleDelete(armId: string) {
    ...
    try {
      ...
      // Query invalidation and toast are handled by the mutation hook
    } catch (error) {
  ...
  async function handleReactivate(armId: string) {
    setIsLoading(true)
    try {
      await reactivateClasses(armId)
      setToReactivate(undefined)
    } catch {
      console.log("Failed to reactivate class")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  async function handleConfirmReactivate(armId: string) {
    if (!armId) {
      return
    }
    setToReactivate(armId)
  }
