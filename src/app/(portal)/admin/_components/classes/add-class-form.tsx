...
const AddClassForm = ({ onSubmit, isLoading, defaultValues }: AddClassFormProps) => {
  ...
  useEffect(() => {
    if (!isLoadingSession && currentSession && !defaultSession) {
      // Use setValue instead of reset to avoid overwriting other fields
      // Ensure we always set a string value
      const sessionName = currentSession.name || ""
      if (sessionName && academicSessionValue !== sessionName) {
        setValue("academicSession", sessionName, { shouldValidate: false })
      }
    }
  }, [isLoadingSession, currentSession, defaultSession, setValue, academicSessionValue])
