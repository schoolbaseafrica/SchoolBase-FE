...
export const NewPersonFormBuilder: React.FC<FormBuilderProps> = ({
  ...
  // Update form data when initialData changes (for edit mode when data loads asynchronously)
  useEffect(() => {
    if (initialData && Object.keys(initialData).length > 0) {
      const currentInitialDataStr = JSON.stringify(initialData)
      // Only update if the initialData has actually changed
      if (previousInitialDataRef.current !== currentInitialDataStr) {
          ...
          <div>
            <div className="relative">
              <input
                ...
                max={maxDate}
                className={cn(inputClasses, "cursor-pointer pr-10")}
              />
