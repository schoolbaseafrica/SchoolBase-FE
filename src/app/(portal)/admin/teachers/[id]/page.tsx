...
import { ArrowLeftIcon, AlertCircle, Users, ChevronDown } from "lucide-react"
...
import { useUnassignTeacherFromClass } from "../_hooks/use-unassign-teacher-from-class"
...
export default function EditTeacherPage() {
  ...
  const [unassigningClassId, setUnassigningClassId] = useState<string | null>(
    null
  )
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              View or unassign classes. Uncheck to remove assignment.
            </p>
          </div>
        </div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                ...
                role="combobox"
                className={cn(
                  "min-w-[220px] justify-between gap-2 font-normal",
                  !assignedClasses?.length && "text-muted-foreground"
                )}
              >
            ...
            >
              <div className="max-h-[280px] overflow-y-auto py-1">
                {(assignedClasses ?? []).map((classItem) => {
                  const sessionId = classItem.academicSession?.id
                  const isUnassigning = unassigningClassId === classItem.id
                  return (
                    <label
                      key={classItem.id}
                      className={cn(
                        "flex cursor-pointer items-center gap-3 rounded-sm px-3 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground",
                        isUnassigning && "pointer-events-none opacity-60"
                      )}
                    >
                      <Checkbox
                        checked={true}
                        ...
                        onCheckedChange={async (checked) => {
                          ...
                          setUnassigningClassId(classItem.id)
                          try {
                      ...
                      <div className="flex flex-1 items-center gap-2 overflow-hidden">
                        ...
                        {classItem.academicSession && (
                          <span className="truncate text-muted-foreground">
                            {classItem.academicSession.name}
                          </span>
                        )}
                      </div>
                    </label>
