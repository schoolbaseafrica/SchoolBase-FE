...
export const AcademicSessionAPI = {
  ...
  getActive: async () => {
    try {
      // Use axios directly with validateStatus to treat 404 as non-error
      const axios = (await import("axios")).default
      const url = "/api/proxy-auth/academic-session/active"
      ...
      // If 404, return null (no active session) - this is expected, not an error
      if (res.status === 404) {
        return null as any
      }

      const result = res.data as ApiAcademicSession | { data: ApiAcademicSession }
      return extractSession(result)
    } catch (error: any) {
      // Fallback: Handle 404 (no active session) gracefully if validateStatus didn't work
      if (
